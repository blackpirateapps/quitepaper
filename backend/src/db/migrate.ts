...
export async function runMigrations(db: Client): Promise<void> {
  ...
  try {
    await db.execute("ALTER TABLE attachments ADD COLUMN status TEXT NOT NULL DEFAULT 'referenced';");
  } catch (_) {}
  try {
    await db.execute("ALTER TABLE attachments ADD COLUMN orphaned_at TEXT;");
  } catch (_) {}
  try {
    await db.execute("ALTER TABLE documents ADD COLUMN status TEXT NOT NULL DEFAULT 'referenced';");
  } catch (_) {}
  try {
